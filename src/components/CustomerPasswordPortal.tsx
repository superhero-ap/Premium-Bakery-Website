    ...
    try {
      if (method === 'phone') {
        ...
        if (mode === 'signup') {
          ...
          if (!data.session) throw new Error('Account was created, but this project is asking for email confirmation. Please disable email confirmation in Supabase Auth settings for password-only signup.')
          const { error: profileError } = await supabase.from('customer_profiles').upsert({ id: data.user.id, full_name: name.trim(), phone: `+91${mobile}` }, { onConflict: 'id' })
        ...
        const { data, error } = await supabase.auth.signUp({ email: normalizedEmail, password, options: { data: { full_name: name.trim(), phone: mobile ? `+91${mobile}` : '', login_method: 'email' } } })
        if (error) throw error
        if (!data.session) throw new Error('Account was created, but this project is asking for email confirmation. Please disable email confirmation in Supabase Auth settings for password-only signup.')
      ...
      setMessage(error instanceof Error ? error.message : 'Unable to create the account. Please try again.')
