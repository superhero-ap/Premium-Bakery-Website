...
const cleanIndianPhone = (value: string) => value.replace(/\D/g, '').replace(/^91/, '')
const toE164 = (value: string) => `+91${cleanIndianPhone(value)}`
  ...
  const submitAuth = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (method === 'phone') {
        ...
        if (mode === 'signup') {
          const { data, error } = await supabase.auth.signUp({ phone: e164, password, options: { data: { full_name: name.trim(), phone: e164 } } })
          if (error) throw error
          if (!data.session) throw new Error('Mobile account was created, but mobile verification is enabled on this project. Disable phone confirmation for password-only login, then try again.')
          await supabase.from('customer_profiles').upsert({ id: data.user.id, full_name: name.trim(), phone: e164 }, { onConflict: 'id' })
        ...
        const { data, error } = await supabase.auth.signUp({ email: email.trim(), password, options: { data: { full_name: name.trim(), phone: cleanIndianPhone(phone) } } })
        if (error) throw error
        if (!data.session) throw new Error('Account was created, but email confirmation is enabled. Disable email confirmation for password-only login, then try again.')
        await supabase.from('customer_profiles').upsert({ id: data.user.id, full_name: name.trim(), phone: cleanIndianPhone(phone) }, { onConflict: 'id' })
        setMessage('Account created successfully.')
      } else {
