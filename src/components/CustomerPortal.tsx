...
export default function CustomerPortal({ onClose }: Props) {
  ...
  const submitAuth = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (method === 'phone') {
        ...
        if (error) throw error
        if (data.user && data.session) await supabase.from('customer_profiles').upsert({ id: data.user.id, full_name: name.trim(), phone: cleanPhone }, { onConflict: 'id' })
  ...
  const saveOrderEdit = async (order: Order) => {
    ...
    setMessage(error ? error.message : 'Order update saved.')
    if (!error && session?.user) await loadAccount(session.user.id)
    if (!error) setDiscountOrder(null)
    setBusy(false)
  }
