...
export default function CustomerPortal({ onClose }: Props) {
  ...
  const [orders, setOrders] = useState<Order[]>([]); const [cashback, setCashback] = useState(0); const [discountNote, setDiscountNote] = useState(''); const [discountOrder, setDiscountOrder] = useState<string | null>(null); const [message, setMessage] = useState(''); const [busy, setBusy] = useState(false)
  ...
  const saveOrderEdit = async (order: Order, scheduledDate: string, scheduledTime: string, note: string) => {
    if (!supabase) return
    setBusy(true); setMessage('')
    const { error } = await supabase.rpc('update_customer_order', { payload: { orderId: order.id, scheduledDate, scheduledTime, customerNote: note } })
    if (error) setMessage(error.message); else { setMessage('Order update saved.'); if (session?.user) await loadAccount(session.user.id) }
    setBusy(false)
  }
